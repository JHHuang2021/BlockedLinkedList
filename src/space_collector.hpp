...
class SpaceCollector {
 ...
 public:
  SpaceCollector(std::string file_name_) {
    disk_manager_ = new DiskManager(file_name_, valid_block_, VALID_BLOCK_ADDR, last_block_, LAST_BLOCK_ADDR);
  ...
  ~SpaceCollector() {
    ...
    for (int i = 0; i < valid_block_; i++) disk_manager_->Write(&block_index_[i], (i + 2) * SIZE_INT);
